...
tankListFromType.addEventListener("click", (event) => {
  ...
  for (let i = 0; i < tanksArray.length; i++) {
    writeTanks.innerHTML += `<div class='tank_info'><div>${tanksArray[i].national}</div> <div>${tanksArray[i].type}</div> <div><img src ='${tanksArray[i].icon}'></div><div>${tanksArray[i].name}</div><div> level: ${tanksArray[i].level}</div></>`;
  }
});
